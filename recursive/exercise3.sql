...
WITH RECURSIVE recommenders(member, recommender) AS (
    ...
    FROM
        cd.members AS m1
    UNION ALL
    SELECT r.member, m2.recommendedby
    FROM
        recommenders AS r
        JOIN cd.members AS m2 ON m2.memid = r.recommender
                                                    )

SELECT r.member, r.recommender, m.firstname, m.surname
FROM
    recommenders AS r
    JOIN cd.members AS m ON r.recommender = m.memid
WHERE
    r.member = 12 OR r.recommender = 22
ORDER BY
    r.member, r.recommender DESC;