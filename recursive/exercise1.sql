...
VALUES
    ...
    (22, 'Coplin', 'Joan', '85 Bard Street, Bloomington, Boston', 43533, '(822) 499-2232', 16,
     '2012-08-29 08:32:41'),
    ...
    (28, 'Farrell', 'David', '437 Granite Farm Road, Westford', 43532, '(855) 755-9876', NULL,
     '2012-09-15 08:22:05'),
    (29, 'Worthington-Smyth', 'Henry', '55 Jagbi Way, North Reading', 97676, '(855) 894-3758', 2,
     '2012-09-17 12:27:15'),
    (30, 'Purview', 'Millicent', '641 Drudgery Close, Burnington, Boston', 34232, '(855) 941-9786', 2,
     '2012-09-18 19:04:01'),
    (33, 'Tupperware', 'Hyacinth', '33 Cheerful Plaza, Drake Road, Westford', 68666, '(822) 665-5327', NULL,
     '2012-09-18 19:32:05'),
...
WITH RECURSIVE recommenders(rec) AS (
    SELECT members.recommendedby
    FROM
        members
    WHERE members.memid = 27
    UNION ALL
    SELECT m.recommendedby
    FROM
        recommenders AS r
        JOIN members AS m ON m.memid = r.rec
                                    )
SELECT r.rec, m.firstname, m.surname
FROM
    recommenders AS r
    JOIN members AS m ON r.rec = m.memid
ORDER BY
    m.memid DESC;
