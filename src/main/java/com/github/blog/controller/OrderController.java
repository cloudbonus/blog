...
public class OrderController {
    ...
    public OrderDto reserve(@RequestBody @P("id") @Positive Long id) {
        return orderService.reserve(id);
    }

    @GetMapping("{id}/cancel")
    @PreAuthorize("hasRole('ADMIN') or (hasRole('COMPANY') and #id == authentication.principal.id)")
    public OrderDto cancel(@PathVariable("id") @P("id") @Positive Long id) {
        return orderService.cancel(id);
    }

    @GetMapping("{id}/buy")
    @PreAuthorize("hasRole('ADMIN') or (hasRole('COMPANY') and #id == authentication.principal.id)")
    public OrderDto buy(@PathVariable("id") @P("id") @Positive Long id) {
        return orderService.buy(id);
    }

    @GetMapping("{id}")
    @PreAuthorize("hasRole('ADMIN') or (hasRole('COMPANY') and #id == authentication.principal.id)")
    public OrderDto findById(@PathVariable("id") @Positive Long id) {
        return orderService.findById(id);
    }

    @GetMapping
    @PreAuthorize("hasRole('ADMIN') or (hasRole('COMPANY') and @orderAccess.canFilter(#filter.userId))")
    public PageResponse<OrderDto> findAll(@P("filter") @Valid OrderFilterRequest filter, @Valid PageableRequest pageableRequest) {
        return orderService.findAll(filter, pageableRequest);
    }

    @PutMapping("{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public OrderDto update(@PathVariable("id") @Positive Long id, @RequestBody @Valid OrderRequest orderDto) {
        return orderService.update(id, orderDto);
    }

    @DeleteMapping("{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public OrderDto delete(@PathVariable("id") @Positive Long id) {
        return orderService.findById(id);
    }
}
