...
public class UserRequest {
    ...
    @NotBlank(message = "Username is mandatory", groups = BaseMarker.First.class)
    private String username;

    @ValidPassword
    @NotBlank(message = "Password is mandatory", groups = BaseMarker.First.class)
    private String password;

    @UniqueEmail(groups = BaseMarker.Third.class)
    @Pattern(message = "Invalid email", regexp = "^[A-Za-z][a-zA-Z0-9_!#$%&'*+/=?`{|}~^.-]+@[a-zA-Z0-9.-]+$")
    @NotBlank(message = "Email cannot be empty", groups = BaseMarker.Second.class)
