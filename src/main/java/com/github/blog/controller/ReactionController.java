...
public class ReactionController {
    ...
    public ReactionDto create(@RequestBody @Valid ReactionRequest request) {
        return reactionService.create(request);
    }

    @GetMapping("{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ReactionDto findById(@PathVariable("id") @Positive Long id) {
        return reactionService.findById(id);
    }

    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    public PageResponse<ReactionDto> findAll(@Valid PageableRequest pageableRequest) {
        return reactionService.findAll(pageableRequest);
    }

    @PutMapping("{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ReactionDto update(@PathVariable("id") @Positive Long id, @RequestBody @Valid ReactionRequest request) {
        return reactionService.update(id, request);
    }

    @DeleteMapping("{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ReactionDto delete(@PathVariable("id") @Positive Long id) {
